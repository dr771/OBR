...
4. **Launch = a store-to-store migration, not a theme publish** (Nick's setup, confirmed 2026-08-11). There are **two separate Shopify shops**: this dev shop, which never goes public under the real domain, and a separate live shop created at launch. So nothing here is customer-facing and no theme push on this store needs treating as a release — work on whichever theme is convenient (the swatch/facet work is on Dawn `148245381229` as of 2026-08-11).
   ...
   **The trap to plan for:** a theme copy carries *only* the theme. It does **not** carry the things this build depends on — `filtercolors`/`activities` **metaobject definitions and entries**, the **metafield definitions** (`custom.filtercolors`, `custom.genderid`, `akeneo.available_erp_sizes`…), or the **Search & Discovery filter config** (Kleur/Maat/Merk/Gender). Those must be recreated on the live shop, and metaobject entries must be **ACTIVE** there (see D3 — DRAFT silently removes the whole colour filter). Worth scripting the metaobject/metafield definition export rather than rebuilding by hand.
