...
- Raw *option* values (`[shoe_size_eu]`) do arrive unsorted, so anything rendering options directly (PDP size pickers) still owns its ordering. `akeneo.available_erp_sizes` as a stored metafield is only lexically sorted (`L M S XL` — wrong for letters); don't read it directly as a sorted source.
