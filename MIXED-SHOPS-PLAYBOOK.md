...
The existing `plp-color-filter` capability now ports the missing visual half of SB’s pattern without importing its client-side family merge: OB already receives one native `custom.filtercolors` value per Akeneo-managed family. Desktop renders 2.8rem chips in five fixed columns, with ten values forming two complete rows before Dawn’s native “Meer weergeven”; the current mobile drawer renders all values in the same five-column grid. Labels and counts move out of permanent view into a CSS-only hover/keyboard-focus tooltip while remaining real visually hidden text inside each native checkbox label. Active pills show only the value (`blue`, not `Kleur: blue`).

Verified live on desktop and a true 390 px touch viewport: exact 28px/12px geometry, twelve accessible checkbox names, keyboard focus and tooltip, immediate checked state, AJAX persistence to four blue products, value-only desktop/mobile pills, three-row show-more expansion, no horizontal overflow, and a 200 section response. Touch hover is explicitly suppressed because mobile Chrome retains `:hover` after a tap; keyboard `:focus-visible` remains supported. The full `plp-mobile-filter-bar` and `plp-filter-panel-chrome` stay separate/deferred, and there is no shop/admin dependency.
...
1. **Port next from the remaining ready reuse candidates:** `plp-mobile-filter-bar`, `predictive-search-overlay`, `cart-drawer-line-item-layout`, or `wishlist-integration`. `plp-filter-panel-chrome` is deliberately deferred at the owner's request.
