...
1. **Port next from the reuse ledger:** `plp-filter-panel-chrome` and `plp-mobile-filter-bar` are the natural pair now that the collection page uses the vertical layout and the mobile drawer is still stock Dawn. Then: `plp-grid-config` (including load-more), `plp-loading-feedback`, `plp-scroll-clamp`, `plp-sort-options`, `predictive-search-overlay`, `cart-drawer-line-item-layout`, and `wishlist-integration`.
