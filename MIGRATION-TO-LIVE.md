...
- [ ] **Add the `display_label` field to the `filtercolors` and `activities` metaobject definitions, and populate it on every entry.** The facet labels are rendered by `snippets/ob-facet-value-label.liquid`, which reads `display_label` — *not* `label`. It cannot read `label`, because **`label` is a reserved property on Shopify's metaobject Liquid drop** and returns the entry's handle instead of the field (verified live: `pink` entry → `hexcode` `#FFC0CB` ✅, `code` `pink` ✅, `label` `pink` ❌ where the stored value is `roze`). Without this field the snippet falls back to Shopify's filter-index label, i.e. English colour and activity names on a Dutch storefront. Added and backfilled on dev 2026-08-26 (16 `filtercolors` + 7 `activities`); definitions and values do not travel with the theme. Ideally the Akeneo sync writes it — see NICK.md item 4.
