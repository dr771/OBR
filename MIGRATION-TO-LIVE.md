...
- [ ] **Set Dutch as the shop's PRIMARY locale before creating any content.** **Done on dev 2026-09-20 — repeat on live, and do it FIRST.** Dev ran with `en` primary and `nl` merely published, while the EU market already served `nl` at the root URL. The measured cost was severe: the Dutch storefront's Category facet covered 16 of 565 products, Gender 17 of 565 and Maat 330 of 3452, while `/en` covered 564, 565 and 3452. Every broken facet was a **plain-text** metafield; the metaobject-reference facets (Kleur, Activities) and native `vendor` were identical on both locales. Content rendering falls back to the primary locale automatically, the storefront **filter index does not** — plain-text metafield values only facet reliably in the primary locale. Flipping the default fixed all three **instantly, with no reindex wait**.

  Three things to know before repeating it. Shopify's primary locale can only be changed in the admin (Settings → Languages → English row → More actions → **Change default**), never via the Admin API. The confirm dialog shows a red *"Your Dutch translations will be deleted"* warning — harmless only when the target language genuinely has no translations, so verify first (`translations(locale: "nl")` empty on sampled resources, and the Languages page showing "No translations"). And the flip **removes the old primary locale entirely** rather than demoting it: after the change `shopLocales` listed only `nl`, and `/en/` returned 404. Re-add English afterwards via *Add language* if the DE/FR/LU market needs it. The theme's `locales/en.default.json` / `nl.json` need **no** rename — verified on the live theme after the flip, the storefront renders Dutch and `en.default.json` stays as a harmless fallback.

- [ ] **Recreate the category-first `Main menu` and its collection targets.** On dev, the menu has seven top-level links in this exact order: Sport & Training, Outdoor & Werk, Fashion & Lifestyle, Schoenen, Kleding, Accessoires, Merken. Sport & Training, Schoenen, Kleding, Accessoires, and Merken have the second-level children documented in `COLLECTIONS.md`; Solden stays out while empty. Recreate and publish the 18 category and six activity smart collections after the catalog sync, then set `custom.breadcrumb_rank = 5` on each. Convert/create the Accessoires parent with its documented four-rule OR set and rank 10. **Merken** points to the published Shopify page `/pages/merken`, which uses `templates/page.merken.json`; create that page, the vendor smart collections, and set its template suffix to `merken`, then add every current brand as a child. Navigation, collections, publication state, and metafield values are shop data, so none travels with a theme push. In the Header section, select desktop menu type **Dropdown**. Fine-pointer hover behavior comes from the theme's `assets/details-disclosure.js`; touch and keyboard stay click/Enter based.
- [ ] **Recreate the three special automatic collections from `COLLECTIONS.md`.** `Sport & Training`, `Outdoor & Werk`, and `Fashion & Lifestyle` (renamed from `Dagelijks Comfort` 2026-09-03) each use an OR rule set over Product **vendor** (`VENDOR`/`EQUALS`, not metafields — see `COLLECTIONS.md`'s "Special collections" for the exact brand-per-collection mapping and why this replaced the original activity/category rule set). Recreate their approved mapping after the live catalog sync. **Verify the RH+ and Magnum vendor strings against the real live product records** — these were unverified estimates on dev since neither brand had synced there yet. Publish each collection to Online Store.
...
- [ ] **Recreate the Category filter on `custom.shopify_originalbrands_category`.** This replaces the abandoned Shopify taxonomy source. On dev it now exposes the real Akeneo values and covers 564 of 565 active products (verified 2026-09-21); the exact current vocabulary and remaining data defects are documented in `COLLECTIONS.md` and `NICK.md` #11.
...
- [ ] **Create the `custom.breadcrumb_rank` collection metafield definition and set its values.** Integer, owner **Collection**, storefront access **public read**. Per `pdp-breadcrumb`, when a product sits in several collections the PDP breadcrumb picks the lowest rank; unset sorts last. This is what stops the breadcrumb naming the broadest collection on every product — on dev, `Dagelijks Comfort` (18 of 26 products) won every trail before this existed, because Liquid's `product.collections` order is neither documented nor configurable. Created on dev 2026-08-26 with this banding, which is merchant data and re-editable in Admin without a deploy:
  - **5** — second-level product category/activity: all 24 menu collections documented in `COLLECTIONS.md`
