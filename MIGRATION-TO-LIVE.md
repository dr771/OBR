...
- [ ] **Metaobject entries must be `ACTIVE`, not `DRAFT`.** DRAFT entries are invisible to the storefront, which silently removes the *entire* colour filter — it still looks perfect in the admin. This bit us on dev (see playbook D3 / `NICK.md` #1). Verify after the first sync:
  ```graphql
