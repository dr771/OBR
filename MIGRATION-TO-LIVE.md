...
- [ ] **Set Dutch as the shop's PRIMARY locale before creating any content.** On dev the primary locale is `en` with `nl` merely published, while the EU market's web presence already serves `nl` at the root URL and `en` at `/en/` — so customers see a language the shop does not author in. That mismatch has a measured cost: on 2026-09-20 the Dutch storefront's Category facet covered 16 of 565 products and Gender 17 of 565, while the same facets on `/en` covered 564 and 565. Every broken facet is a **plain-text** metafield; the metaobject-reference facets (Kleur, Activities) and native `vendor` are identical on both locales. Content rendering falls back to the primary locale automatically, but the storefront **filter index** evidently does not, so plain-text metafield values only facet reliably in the primary locale. Shopify's primary locale can only be changed in the admin (Settings → Languages), never via the Admin API, and it re-anchors existing translations — which is exactly why it must be set on the live shop *before* content exists, not retrofitted. Keep `en` as a published alternate for the DE/FR/LU market. Also rename the theme's `locales/en.default.json` → `nl.default.json` (and `en.json`) so the theme's own default language matches. See NICK.md item 10.
